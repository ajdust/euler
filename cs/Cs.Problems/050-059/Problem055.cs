...
namespace Cs.Problems
{
    ...
    {
        public bool IsLychrel(BigInteger x)
        {
            // apply initial transform to move past already-palindromic 
            x += BigInteger.Parse(new string(x.ToString().Reverse().ToArray()));

            for (var i = 1; i <= 49; i++)
            {
                var digits = new string(x.ToString().Reverse().ToArray());
                if (digits == x.ToString())
                    return false;

                x += BigInteger.Parse(digits);
            }
