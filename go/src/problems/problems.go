...
func AtOrDefault(g [20][20]int, defaultValue int, x int, y int) int {
	height := 20
	width := 20
...
func GetLargestAdjacent(g [20][20]int, x int, y int) int {
	right := AtOrDefault(g, 1, x, y) *
		AtOrDefault(g, 1, x+1, y) *
		AtOrDefault(g, 1, x+2, y) *
		AtOrDefault(g, 1, x+3, y)

	down := AtOrDefault(g, 1, x, y) *
		AtOrDefault(g, 1, x, y+1) *
		AtOrDefault(g, 1, x, y+2) *
		AtOrDefault(g, 1, x, y+3)

	downright := AtOrDefault(g, 1, x, y) *
		AtOrDefault(g, 1, x+1, y+1) *
		AtOrDefault(g, 1, x+2, y+2) *
		AtOrDefault(g, 1, x+3, y+3)

	downleft := AtOrDefault(g, 1, x, y) *
		AtOrDefault(g, 1, x-1, y+1) *
		AtOrDefault(g, 1, x-2, y+2) *
		AtOrDefault(g, 1, x-3, y+3)
...
func Problem11() string {
	...
	grid := [20][20]int{
		[20]int{8, 02, 22, 97, 38, 15, 00, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8},
...
func GetFactors(knownFactors map[int64][]int64, n int64) []int64 {
	...
	// 5 lines to get the keys of a map
	// how is this still a thing programming in 2018?
